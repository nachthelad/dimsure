...
import { normalizeProductName, validateProductName } from "@/lib/product-normalizer"
...
export function AddProductForm() {
  ...
  const validateField = (field: keyof ValidationErrors, value: string): string | undefined => {
    switch (field) {
      case 'name':
        ...
        if (value.trim().length < 3) return t("addProduct.validation.nameMinLength")
        if (value.trim().length > 100) return t("addProduct.validation.nameMaxLength")
        break
      case 'sku':
        if (!value.trim()) return t("addProduct.validation.skuRequired")
        if (value.trim().length < 2) return t("addProduct.validation.skuMinLength")
        if (value.trim().length > 50) return t("addProduct.validation.skuMaxLength")
        break
      case 'brand':
      ...
      case 'category':
        ...
        break
      case 'weight':
        if (value.trim() && (Number.parseFloat(value) <= 0 || isNaN(Number.parseFloat(value)))) {
  ...
  const validateDimensions = (): string | undefined => {
    if (!formData.length.trim() || !formData.width.trim() || !formData.height.trim()) {
  ...
  const handleImagesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of newFiles) {
      ...
      if (!allFiles.some(f => f.name === file.name && f.size === file.size)) {
        ...
        if (!validation.isValid) {
          setValidationErrors(prev => ({ ...prev, images: validation.error }));
          continue;
        }
        allFiles.push(file);
      }
    ...
    if (allFiles.length > 3) {
      setValidationErrors(prev => ({ ...prev, images: t("addProduct.validation.maxImages") }));
      allFiles.length = 3;
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.message.includes("already exists")) {
        setValidationErrors(prev => ({ ...prev, sku: t("addProduct.validation.skuExists") }))
      } else {
