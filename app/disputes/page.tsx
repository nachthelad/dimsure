...
const DisputeModal = dynamic(
  () =>
    import("@/components/features/dispute-modal").then((mod) => ({
      default: mod.DisputeModal,
    })),
  {
...
export default function DisputesPage() {
  ...
  useEffect(() => {
    ...
    if (typeFilter !== "all") {
      filtered = filtered.filter(
        (dispute) => dispute.disputeType === typeFilter
      );
    }
  ...
  const loadDisputes = async () => {
    try {
      ...
      const enrichedDisputes: Dispute[] = await Promise.all(
        disputesData.map(async (dispute): Promise<Dispute> => {
        ...
        <div className="space-y-4">
          {filteredDisputes.map((dispute) => {
            ...
            if (
              ...
              dispute.resolutionPendingAt &&
              dispute.productCreatedBy &&
              dispute.productLastModified
            ) {
              const pendingAtMs = dispute.resolutionPendingAt.toDate
                ? dispute.resolutionPendingAt.toDate().getTime()
                : new Date(dispute.resolutionPendingAt).getTime();
              const lastModifiedMs = dispute.productLastModified.toDate
                ? dispute.productLastModified.toDate().getTime()
                : dispute.productLastModified
                ? new Date(dispute.productLastModified).getTime()
                : 0;
              const now = Date.now();
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        ...
                        >
                          {readableId}{" "}
                          {dispute.title || t("disputes.dispute.untitled")}
                        </a>
                      ...
                      <p className="text-sm text-muted-foreground mb-2">
                        ...
                        <span className="font-mono">
                          {dispute.productSku || "N/A"}
                        </span>{" "}
                        -{" "}
                        {dispute.productName ||
                          t("disputes.dispute.unknownProduct")}
                      </p>

                      <p className="text-muted-foreground mb-4">
                        {dispute.description ||
                          t("disputes.dispute.noDescription")}
                      </p>
