...
export default function ProductDetailPage({
  ...
  const [categoryTranslation, setCategoryTranslation] = useState<string | null>(
    null
  );
  ...
  const handleSuggestDifferent = () => {
    ...
    const initialData = {
      title: t("product.suggestedCorrectionTitle", {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        if (productData) {
          ...
          if (
            productData.lastModifiedBy &&
            productData.lastModifiedBy !== productData.createdBy
          ) {
          ...
          if (isLoggedIn && user) {
            await incrementProductViews(resolvedParams.slug);
          }
        } else {
          notFound();
        }
    ...
    fetchProduct();
  }, [resolvedParams.slug, user, isLoggedIn]);
  ...
  const handleLike = async () => {
    ...
    try {
      if (isLiked) {
        await unlikeProduct(user.uid, resolvedParams.slug);
        setIsLiked(false);
        setLikesCount((prev) => prev - 1);
      } else {
        await likeProduct(user.uid, resolvedParams.slug);
        setIsLiked(true);
        setLikesCount((prev) => prev + 1);
      }
    } catch (error) {
